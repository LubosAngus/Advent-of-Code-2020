...
const getCounts = (bags, color, result = []) => {
  const bagContents = bags[color]

  for (const bagColor in bagContents) {
    const bagCount = bagContents[bagColor]
    const bbagCount = bagCount + getCounts(bags, bagColor, result)

    result = bagCount * bbagCount
...
fs.readFile(`${demo ? 'demo_01' : 'input'}.txt`, 'utf8', (err, data) => {
  ...
  let countOfBag = getCounts(bags, 'shiny_gold')

  console.log(bags)
  console.table(countOfBag)
  console.timeEnd('run')
})
